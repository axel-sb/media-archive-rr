...
export const loader: LoaderFunction = async ({ params }) => {
  ...
  return {
    id: params.id,
    filename: `Image_${params.id}.jpg`,
    fileType: "image/jpeg",
    description: "A beautiful landscape image showing mountains and a lake.",
    ...
    altitude: 41.24,
    views: 12,
    googleUrl: "https://photos.google.com/photo/example",
    deviceType: "IOS_PHONE",
    tags: ["nature", "landscape", "vacation"]
  } as MediaDetailData;
};
        ...
        <div className="md:flex">
          ...
          <div className="md:w-2/3 bg-black flex items-center justify-center">
            <div className="aspect-video w-full h-full bg-gray-900 flex items-center justify-center text-gray-400">
              ...
              <div className="text-center p-4">
                ...
                <p className="mt-4">Media {media.id}</p>
              </div>
            </div>
          </div>
          ...
          <div className="md:w-1/3 p-6">
            <div className="flex justify-between items-start mb-4">
              <h2 className="text-xl font-bold text-gray-800">{media.filename}</h2>
              <div className="text-sm text-gray-500">ID: {media.id}</div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-6">
              <div>
                ...
                <p className="text-gray-800">{new Date(media.createdAt).toLocaleDateString()}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500">Taken</h3>
                <p className="text-gray-800">{new Date(media.takenAt).toLocaleDateString()}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500">File Type</h3>
                <p className="text-gray-800">{media.fileType}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500">Device</h3>
                <p className="text-gray-800">{media.deviceType}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500">Views</h3>
                <p className="text-gray-800">{media.views}</p>
              </div>
            </div>
              ...
              <div className="mb-6">
                ...
                <div className="aspect-video bg-gray-200 rounded-md flex items-center justify-center text-gray-400">
                  ...
                  <div className="text-center">
                    <p>{media.latitude.toFixed(4)}, {media.longitude.toFixed(4)}</p>
                    <p className="text-xs">Altitude: {media.altitude}m</p>
                  </div>
                </div>
              </div>
            )}
