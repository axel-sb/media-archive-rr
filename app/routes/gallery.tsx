...
export default function Gallery() {
	...
	return (
		<div
			...
			style={{
				backgroundImage:
					'linear-gradient(45deg, #f0f,#0004,#0009, #0006), linear-gradient(105deg, #0005,#0004,#0006, #0008)',
			}}
		>
			<div className="rounded-xs bg-[#000000bb] py-6 shadow-md backdrop-blur-md backdrop-brightness-[0.25] backdrop-sepia">
				<div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
					<h2 className="text-2xl font-bold text-gray-300">Media Gallery</h2>

					<div className="flex gap-2">
						<select className="rounded-md border border-gray-700 px-3 py-2 focus:border-blue-500 focus:ring-blue-500">
                ...
                */}
				...
				<div className="grid w-full grid-cols-6 gap-2 text-base sm:grid-cols-9 md:grid-cols-12 md:gap-2 lg:grid-cols-15 lg:gap-2 grid-rows-[masonry]">
						...
						<div key={photo.uuid} className="group relative">
							<div className="aspect-square overflow-hidden rounded-md bg-gray-200 transition-opacity hover:opacity-80">
								<div className="flex h-full w-full items-center justify-center text-gray-400">
									{relPath[index] && (
										<img src={`images/${relPath[index]}`} className="w-full h-full object-cover" />
									)}
								</div>
							</div>
